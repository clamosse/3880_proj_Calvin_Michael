...
boosted_mse
# Confusion matrix and accuracy for boosting model
boosted_threshold <- 0.5
boosted_predicted_class <- ifelse(boosted_predictions > boosted_threshold, 1, 0)
boosted_confusion_matrix <- table(boosted_predicted_class, ifelse(test_data$SNWD_change > 0, 1, 0))

boosted_correct_pred <- sum(diag(boosted_confusion_matrix))
boosted_total_pred <- sum(boosted_confusion_matrix)

boosted_accuracy <- boosted_correct_pred / boosted_total_pred
...
confusion_df <- as.data.frame.table(boosted_confusion_matrix)
